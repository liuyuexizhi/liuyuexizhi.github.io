...
以下是原仓库内容，个人添加了 `src` 目录来作为上传文件入口，以及在 `blog.sh` 脚本 `deploy` 参数中添加了自动处理文章和资源文件的脚本。
...
文章资源放在`posts_imgs`目录，如文章文件名是`2019-05-01-theme-usage.md`，则该篇文章的资源需要放在`posts/2019/05/01`下,在文章使用时直接引用即可。当然了，写作的时候会提示资源不存在忽略即可
